...
<div class="container py-4">
    <div class="card p-4 mb-4 border-0" style="background: linear-gradient(135deg, #071120 0%, #13294d 100%); color: white;">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
            <div>
                <h2 class="fw-bold mb-1">Found Items</h2>
                <p class="mb-0 text-white-50">Browse campus-found belongings and help bring them back to their rightful owners.</p>
            </div>
            <a href="{{ route('found-items.create') }}" class="btn btn-findit px-4">
        ...
        @forelse ($foundItems as $item)
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm overflow-hidden" style="border-radius: 18px;">
                    @if ($item->image)
                        <img src="{{ asset('storage/' . $item->image) }}" class="card-img-top" alt="{{ $item->title }}" style="height: 240px; object-fit: cover;">
                    @else
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text text-muted small flex-grow-1">{{ Str::limit($item->description, 90) }}</p>
                        <div class="mb-2">
                        ...
                        <a href="{{ route('found-items.show', $item) }}" class="btn btn-sm btn-findit mt-auto">View Details</a>
                    </div>
                </div>
            </div>
        @empty
            <div class="col-12">
                <div class="text-center py-5 rounded-4 border border-dashed bg-white">
                    ...
                    <p class="text-muted mt-3 mb-0">No found items reported yet.</p>
                </div>
            </div>
        @endforelse
    </div>
